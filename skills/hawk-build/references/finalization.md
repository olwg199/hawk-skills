...
Read when preparing a user-authorized related commit or finalizing a full build. For lightweight builds, only the commit guidance applies; completion follows SKILL.md.
...
At completion, give outcome, verification, record path and stage, item IDs grouped by status, and memory path/status; expand only unresolved items and reproduce the ordered **Next steps** exactly.
